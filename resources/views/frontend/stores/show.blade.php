...
@section('content')
    @php $revews_count = $store->activeReviews->count(); @endphp
                            ...
                            <div class="product__content">
                                <div class="product__gallery">
                                    <div class="product-gallery">
                                        <img class="product-image__img" style="position: relative; margin:auto; width:120px" alt="logo"
                                            @if ($store->logo->first()) 
                                ...
                                <div class="product__info">
                                            ...
                                            <div class="rating">
                                                <div class="rating__body">
                                                    @foreach (range(1, 5) as $index)
                                                        <svg class="rating__star @if ($index <= $store->rating) rating__star--active @endif" width="13px" height="12px">
                                                        ...
                                                        <div class="rating__star rating__star--only-edge @if ($index <= $store->rating) rating__star--active @endif">
                                    ...
                                    <div class="product__description">
                                        Save money whilst shopping with {{ $store->name }} cashback and deals! Simply click get cashback, shop as normal and earn rewards in your
                                        Cashback Reborn account.
                                    </div>
                                </div>
                                <div class="product__footer">
                                    ...
                                    <h5>Cashback</h5>
                                    <a href="#"
                                        @guest class="btn btn-primary" data-toggle="modal" data-target="#signinModal" @else form_id="form_{{ $store->id }}" class="btn btn-primary store_form" @endguest>
                                        Get Cashback
                                    </a>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-4 border text-center py-4 px-3 mt-4">
                                    <h4>
                                        @if ($store->custom_cashback_percentage)
                                            @if ($cashback->type == 'fixed') {{ $cashback->currency }} @endif
                                    ...
                                    @php $substr = explode(',', $cashback->detail); @endphp
                                    <p>{{ $substr[0] }}</p>
                                    ...
                                    <a href="#"
                                        @guest class="btn btn-primary" data-toggle="modal" data-target="#signinModal" @else form_id="form_{{ $cashback->id }}" class="btn btn-primary store_form" @endguest>
                                        Get Cashback
                                    </a>
                                </div>
                            @endforeach
                                            ...
                                            @foreach ($store->vouchers->take(3) as $voucher)
                                                <div class="product-card mb-2 p-4">
                                                    <div class="row">
                                                        ...
                                                        <form action="{{ route('site.exit_click.store') }}" method="POST" id="form_voucher_{{ $voucher->id }}"
                                                            target="_blank">
                                                            @csrf
                                                        ...
                                                        <div class="col-md-4 text-md-right">
                                                            @if ($voucher->promotion_type == 'Coupon' && $voucher->coupon_code)
                                                                <div class="coupon-detail coupon-button-type">
                                                                    <a form_id="form_voucher_{{ $voucher->id }}" url="{{ $voucher->click_url }}" rel="nofollow"
                                                                        data-type="code" class="coupon-button coupon-code @guest voucher_form_guest @else voucher_form @endguest"
                                                                        data-code="{{ $voucher->coupon_code }}">
                                                            ...
                                                            @else
                                                                <a style="width: 145px" href="#"
                                                                    @guest class="btn btn-primary btn-sm" data-toggle="modal" data-target="#signinModal" @else form_id="form_voucher_{{ $voucher->id }}" class="btn btn-primary btn-sm store_form" @endguest
                                                                    target="_blank">
                                                                    Get Deal <i class="fas fa-external-link-alt ml-2"></i>
                                                                </a>
                                                            @endif
                                                            <div>
                                                            ...
                                                            <div class="col-md-4 text-md-right">
                                                                @if ($voucher->promotion_type == 'Coupon' && $voucher->coupon_code)
                                                                    <div class="coupon-detail coupon-button-type">
                                                                        <a form_id="form_voucher_{{ $voucher->id }}" url="{{ $voucher->click_url }}" rel="nofollow"
                                                                            data-type="code"
                                                                            class="coupon-button coupon-code @guest voucher_form_guest @else voucher_form @endguest"
                                                                            data-code="{{ $voucher->coupon_code }}">
                                                                            <span class="code-text" rel="nofollow">{{ $voucher->coupon_code }}</span>
                                                                            <span class="get-code">Get Code</span>
                                                                        </a>
                                        ...
                                        <div class="reviews-view">
                                            <div class="reviews-view__list">
                                                ...
                                                <div class="reviews-list">
                                                    @if ($revews_count)
                                                        <ol class="reviews-list__content" data-count="{{ $revews_count }}" id="reviews"></ol>
                                                    @else
                                                        <div class="review__text">There is no review at the moment. Be the first to add one!</div>
                                                    @endif
                                                </div>
                                                <div class="col text-center mt-4" id="load-more"></div>
                                            </div>
                                            @auth
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <h4>
                            Do you want to receive a cashback at
                            <br> <span class="color-primary">{{ $store->name }}?</span>
                        </h4>

                        <img style="max-width: 100px;" class="mx-auto my-4 d-block" alt="logo"
                            @if ($store->logo->first()) 
                                @if ($store->logo->first()->is_fake)
                                    src="{{ asset('frontend/images/logos/' . $store->logo->first()->image) }}"
                                @else
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="#" onclick="document.getElementById('form_{{ $store->id }}').submit()" class="btn btn-warning btn-dim">
