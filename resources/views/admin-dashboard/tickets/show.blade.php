                                                ...
                                                <ul class="nk-msg-actions">
                                                    <li class="text-right">
                                                        @if ($ticket->status != 'closed')
                                                            <a onclick="$('#close-{{ $ticket->id }}').submit();" style="cursor: pointer"
                                                                class="btn btn-dim btn-sm btn-outline-light"> <em class="icon ni ni-check"></em><span>Mark as
                                                                    Closed</span></a>

                                                            <form action="{{ route('admin.tickets.close', $ticket) }}" id="close-{{ $ticket->id }}" method="POST"
                                                                class="m-0">
                                                                @method('PUT')
                                                                @csrf
                                                ...
                                                <div class="nk-reply-body">
                                                    <div class="nk-reply-entry entry">
                                                        @if ($ticket->ticket_type == 'claim')
                                                            Store: <a href="{{ route('admin.stores.show_store') }}?slug={{ $ticket->store->slug }}">{{ $ticket->store->id }}
                                            ...
                                            @if (count($ticket->replies))
                                                @foreach ($ticket->replies as $reply)
                                                    <div class="nk-reply-item">
                                                        <div class="nk-reply-header">
                                                            <div class="user-card">
                                                                <div class="user-avatar sm @if ($reply->reply_by == 'admin') bg-pink @else bg-blue @endif text-uppercase">
                                                                    <span>{{ $reply->user->first_name[0] }}{{ $reply->user->last_name[0] }}</span>
                                                                </div>
                                                            ...
                                                            <div class="nk-reply-form-editor">
                                                                <form method="POST" action="{{ route('admin.replies.store') }}" class="form-validate">
                                                                    ...
                                                                    <div class="nk-reply-form-field">
                                                                        <textarea class="form-control form-control-simple no-resize" name="reply" placeholder="Hello" required>{{ old('reply') }}</textarea>
                                                                    </div>
                                                                    @error('reply')
