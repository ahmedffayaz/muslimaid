        ...
        <div class="nk-tb-item">
            ...
            <div class="nk-tb-col  text-right">
                @if (($commission->statusMap->status ?? $commission->status) == 'confirmed')
                    <span
                        class="tb-status badge badge-success">{{ $commission->statusMap->status ?? $commission->status }}</span>
                @elseif(($commission->statusMap->status ?? $commission->status) == 'paid')
                    <span
                        class="tb-status badge badge-success">{{ $commission->statusMap->status ?? $commission->status }}</span>
                @elseif(($commission->statusMap->status ?? $commission->status) == 'processing')
                    <span
                        class="tb-status badge badge-info">{{ $commission->statusMap->status ?? $commission->status }}</span>
                @elseif(($commission->statusMap->status ?? $commission->status) == 'failed')
                    <span
                        class="tb-status badge badge-danger">{{ $commission->statusMap->status ?? $commission->status }}</span>
                ...
                @elseif(($commission->statusMap->status ?? $commission->status) == 'processing donation')
                    <span
                        class="tb-status badge badge-warning">{{ $commission->statusMap->status ?? $commission->status }}</span>
                        ...
                        <div class="drodown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <ul class="link-list-opt no-bdr">
                                    <li><a href="{{ route('admin.commissions.edit', $commission) }}"
                                            cashback-id='{{ $commission->id }}' class='cashback-edit'><em
                                                class="icon ni ni-edit"></em><span>Edit</span></a></li>
                                    <li><a href="{{ route('admin.commissions.history', $commission) }}"
                                            class='cashback-history'><em class="icon ni ni-eye"></em><span>Status
                                                History</span></a></li>
                                    <li><a onclick="$('#delete-commission-{{ $commission->id }}').submit();"
                                        ...
                                        <form action="{{ route('admin.commissions.destroy', $commission) }}"
                                            id="delete-commission-{{ $commission->id }}" method="POST" class="m-0">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    </li>
