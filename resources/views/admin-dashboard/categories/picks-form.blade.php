                ...
                <div class="form-control-wrap ">
                    <div class="">
                        <select class="form-control select-2" id="default-06" name="category_id" required>
                            ...
                            <option @if($category->id == $cat->id) selected @endif  value="{{$category->id}}" style="font-weight:bold">{{$category->name}}</option>
                            @if(count($cat->childs))
                                @include('admin-dashboard.categories.child_input_picks',['childs' => $cat->childs,'isEdit'=> 1 ,'category'=>$category,'dashes'=>'~'])
                            @endif
