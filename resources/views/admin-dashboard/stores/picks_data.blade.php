...
<div class="nk-tb-item">
    ...
    <div class="nk-tb-col">
        <span><a href="{{route(getAdminPrefix() . '.stores.show_store')}}?slug={{$pick->slug}}"><div class="tb-lead">{{$pick->id}}</div></a></span><br>
        
    </div>
    <div class="nk-tb-col">
        <span><a href="{{route(getAdminPrefix() . '.stores.show_store')}}?slug={{$pick->slug}}"><div class="tb-lead">{{$pick->name}}</div></a></span>
    ...
    <div class="nk-tb-col">
        <h5><a href='{{$pick->store_url}}' target="_blank" class="a_link"><em class="icon ni ni-link-alt"></em></a></h5>
    </div>
                ...
                <div class="drodown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <ul class="link-list-opt no-bdr">
                            <li><a href="{{route(getAdminPrefix() . '.stores.show_store')}}?slug={{$pick->slug}}"><em class="icon ni ni-edit"></em><span>Edit Store</span></a></li>
                            <li><a  onclick="$('#delete-store-{{$pick->id}}').submit();"  style="cursor: pointer"> <em class="icon ni ni-trash-fill"></em><span>Delete Store</span></a>
                                                    
                            <form action="{{ route(getAdminPrefix() . '.stores.destroy', $pick) }}" id="delete-store-{{$pick->id}}" method="POST" class="m-0">
                                @method('DELETE')
                                @csrf
