...
<div class="nk-modal-action">
    <a href="{{ route(getAdminPrefix() . '.importer.import') }}" class="btn btn-sm btn-mw btn-primary run-importer @if(getImporterYMLSettings('Networks_'. $network->name . '_Importer_Stores') === 0 && getImporterYMLSettings('Networks_'. $network->name . '_Importer_Vouchers' ) === 0 && getImporterYMLSettings('Networks_'. $network->name . '_Importer_Cashbacks' ) === 0) disabled @endif">Run Importer</a>
    <a href="" class="btn btn-sm btn-mw btn-primary save_importer @if(getImporterYMLSettings('Networks_'. $network->name . '_Importer_Stores') === 0 && getImporterYMLSettings('Networks_'. $network->name . '_Importer_Vouchers' ) === 0 && getImporterYMLSettings('Networks_'. $network->name . '_Importer_Cashbacks' ) === 0) disabled @endif">Save for later</a>
</div>