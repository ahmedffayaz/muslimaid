$(document).ready(function() {
    ...
    $(".cash-img img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/upload/images/15648842133707348171616775835.jpg');
    });
    $(".cat-missing-icon").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/cat-missing-icon.jpg');
    });
    $(".all-cat-main-banner img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/categories-banner.png');
    });
    $(".store-cat-main-banner img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/store-cate.jpg');
    });
    $(".store-detail-main-banner img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/store-detail-banner.png');
    });
    $(".all-store-banner img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/stores-banner.png');
    });
    $(".all-store-banner img").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/stores-banner.png');
    });
    $(".missing-icon-user").on("error", function(){
        $(this).attr('src', $("base").attr("href")+'resources/front/images/profile-icon_.png');
    });
...
function getAlertMessages(data=null,form=null) {
    if (data.success){
        var html = '<div class="alert alert-success alert-dismissible">\n' +
            ...
            '    <span class="messageTextSuccess">'+data.message+'</span>\n' +
            '</div>';
        ...
        var html = '<div class="alert alert-danger alert-dismissible">\n' +
            ...
            '    <span class="messageTextError">'+data.message+'</span>\n' +
            '</div>';
    }
    return html;
}
...
function reloadTimeOut(givenTime=null,redirect=null) {
    if (givenTime == ""){ var givenTime = 3000}
    setTimeout(function() {
        if (redirect === null){
...
function signupProcess(form) {
    $(form).validate();
    if ($(form).valid() == true) {
        $.post(base_url + "client/signup_process_action", $(form).serialize(), function(data) {
            var data = JSON.parse(data);
            var alertMessage = getAlertMessages(data,form);
            $(form).find(".messageBox").html(alertMessage);
        ...
        $.post(base_url + "client/login_action", $(form).serialize(), function(data) {
            ...
            if (data.success){
                if (data.user_status == "email_verified"){
                    var redirectUrl = base_url + "client/verify_email/" + data.user_code;
...
function passwordReset(form) {
    ...
    if ($(form).valid() == true) {
        $.post(base_url + "client/reset_password_action", $(form).serialize(), function(data) {
            var data = JSON.parse(data);
            var alertMessage = getAlertMessages(data,form);
            $(form).find(".messageBox").html(alertMessage);
        })
...
$(document).on("change", "#add-order", function() {
    ...
    if(value_order!=''){
        window.location.href = $('#get_current_url').val() + value_sort + "/" + value_order;
    }
});
...
$(document).on("change", "#cat-sort", function() {
    ...
    var sort_by = "id";
    var order_by = "desc";
    window.location.href = $('#get_current_url').val() + sort_by + "/" + order_by + "/"+cat_sort;
});
...
$(document).on("change", "#add-order-stores", function() {
    ...
    value_order = ( this.value );
    if(value_order!=''){
        window.location.href = $('#get_current_url_store').val()+ value_sort + "/" + value_order;
    }
})
...
$(document).on("click", "[data-store-id]", function() {
    ...
    if (user_id > 0) {
        if ($(this).hasClass("showcode")) {
            ...
            $(this).replaceWith($("<input readonly type='text' data-clipboard-text='" + code + "' value='" + code + "' class='form-control copyText' />"));
            parent.find("input").notify("Code is copied successfully.", "success");
            ...
            setTimeout(function() {
                window.open(base_url + "client/exitclick/" + store_id + "/" + store_id + "/store");
            }, 2000);
        } else {
            if (cb == "door"){
                var cashback_id = $(this).attr("data-cashback-id");
                window.open(base_url + "client/exitclick/" + store_id + "/" + cashback_id + "/cb_door");
            } else {
                window.open(base_url + "client/exitclick/" + store_id + "/" + store_id + "/store");
            }
        }
...
$(document).on("submit", ".invite-friend", function(e) {
    ...
    if ($(form).valid() == true) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == "success") {
                    $("#multipleEmails").notify("Invitation email is successfully sent!.", "success");
                    setTimeout(function() {
                        window.location.reload();
                    }, 3000)
                } else {
                    $("#multipleEmails").notify("Something goes wrong. Please check your friend's email.", "warn");
                }
            },
            cache: false,
            contentType: false,
    ...
    if($(form).valid()){
        $.ajax({
            ...
            success: function(data) {
                ...
                var alertMessage = getAlertMessages(data,form);
                $(form).find(".messageBox").html(alertMessage);
                if (data.success){reloadTimeOut(3000);}
            }
        });
    }
})
...
function arrangeMeeting(form) {
    ...
    if ($(form).valid() == true) {
        $.post(base_url + "client/arrange_meeting_action", $(form).serialize(), function(data) {
            ...
            var alertMessage = getAlertMessages(data,form);
            $(form).find(".messageBox").html(alertMessage);
            if (data.success){reloadTimeOut(3000, redirectUrl);}
        })
    } else {
        // alert("form is not valid");
    }
...
$(document).on("click", ".copy-meeting-coupon", function(e) {
    $(".copy-meeting-coupon").prev().notify("Code is copied successfully.", "success");
})

function addCharityCause(form) {
    $(form).validate();
    if ($(form).valid() == true) {
        $.post(base_url + "client/add_charity_cause", $(form).serialize(), function(data) {
            var data = JSON.parse(data);
            var alertMessage = getAlertMessages(data,form);
            $(form).find(".messageBox").html(alertMessage);
            if (data.success){reloadTimeOut(2000);}
        })
    } else {
        // alert("form is not valid");
    }
